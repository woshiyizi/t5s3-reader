...
void EpubReaderChapterSelectionActivity::render(RenderLock&&) {
  ...
  for (int i = 0; i < pageItems; i++) {
    ...
    const std::string chapterName = BaseTheme::truncatedTextForRole(renderer, UI_10_FONT_ID, TextRole::UserContent,
                                                                    item.title.c_str(), contentWidth - 40 - indentSize);

    BaseTheme::drawTextForRole(renderer, UI_10_FONT_ID, TextRole::UserContent, indentSize, displayY,
                               chapterName.c_str(), !isSelected);
  }
