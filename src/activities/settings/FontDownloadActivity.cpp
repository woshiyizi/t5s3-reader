...
bool FontDownloadActivity::activateSelectedItem() {
  if (families_.empty()) {
    return false;
  }
...
bool FontDownloadActivity::onTouchTap(int16_t, int16_t y) {
  ...
  const int touchedIndex = pageStart + row;
  if (touchedIndex < 0 || touchedIndex >= listItemCount()) {
    return false;
  }
